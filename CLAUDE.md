...
2. **点击一个词（聚焦）**：关联词被拉到中心围成圈（orbitR=3.5），无关词淡化到 35%，镜头自动移动，**聚焦词的直接连线变为实线（核心层 opacity=1.0）**
3. **再点同一个词（选中）**：打开右侧详情面板
4. **点空白**：退出聚焦，恢复全局视角，重新公转，连线恢复半透明
5. **动画速度**：所有过渡用 lerp 0.15
6. **连线规格**：三层叠加 Line（opacity 0.10/0.25/0.42），聚焦直接连线变为 0.25/0.55/1.0
